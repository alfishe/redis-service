...
// Make alias for function deprecated in VS2012 runtime
#define wstrtod _wstrtod
...
int replace_setsockopt(SOCKET socket, int level, int optname,
...
int aeWinSocketAttach(SOCKET fd);
int aeWinCloseSocket(SOCKET fd);
int aeWinReceiveDone(SOCKET fd);
int aeWinSocketSend(SOCKET fd, char *buf, int len, int flags,
                    void *eventLoop, void *client, void *data, void *proc);
int aeWinListen(SOCKET sock, int backlog);
SOCKET aeWinAccept(SOCKET fd, struct sockaddr *sa, socklen_t *len);
int aeWinSocketConnect(SOCKET fd, const struct sockaddr *sa, int len);
...
// Intellectual exit from process/service loop
