...
int replace_setsockopt(SOCKET socket, int level, int optname, const void *optval, socklen_t optlen)
{
...
// SCM process name (parent to each system service)
...
BOOL getparentprocessname(char* name)
{
	...
	{
		// Copy process name as return value
		strncpy(name, parentEntry.szExeFile, PROCESS_NAME_LEN);
		result = TRUE;
	}
...
BOOL getparentprocessentry(LPPROCESSENTRY32 entry)
{
	...
	if (getselfprocessentry(&selfEntry))
	{
		...
		result = getprocessentry(entry, selfEntry.th32ParentProcessID);
	}
...
BOOL getprocessentry(LPPROCESSENTRY32 entry, DWORD processID)
{
	...
	currentEntry = Process32First(snapshot, entry);
	...
	while (currentEntry)
	{
		...
		// Move on to the next entry (when we reach end, loop will stop)
		currentEntry = Process32Next(snapshot, entry);
	}
